MFPCA_VD_int=function(Data, M_grid=NULL, m_npcs=NULL){

  # DATA IS EXCPECTED IN A LIST FORMAT IN WHICH EACH ELEMENT OF THE LIST IS A VARIABLE
  ...
  n_var=length(Data)
  N=nrow(Data[[1]])
  npcs=univ_efunctions=vector(mode = "list", length = n_var)
  EE=EE_int=NULL
  ...
  for (ind in 1:n_var) {
    ...
    Variable_long<-Data[[ind]]  %>% reshape2::melt(na.rm=T) %>%
      dplyr::rename(id=Var1,time=Var2, y=value) %>%
      arrange(id,time) %>%
      filter(!is.na(y))

    #set up data frame for Variable ind
    ndays_df<-plyr::ddply(Variable_long,~id,summarize, maxT=length(y)) %>%
      arrange(maxT) %>%
      filter(!(maxT == 1))
    ndays_df$newid<-1:dim(ndays_df)[1]
    Variable_l2<-inner_join(Variable_long,ndays_df,by="id") %>% dplyr::select(-id) %>% 									dplyr::rename(id=newid) %>%
      dplyr::select(id, maxT, time, y) %>%
      arrange(id, time)

    #add mean, ytilde
    fit_Variable<-gam(y~s(time,maxT), data= Variable_l2, method="REML")
    Variable_l2$mean<-as.vector(predict.gam(fit_Variable))
    ...
    pc_grid<-get_pcs_M(gridM, cov_temp, Hz=1, includezero=F, npcs = 5) #pve = 0.99 # pve=0.99 means that the number
    # of PC for the univariate basis will be selected
    # as the minimum for a 99% of variance explained
    ...
    aux_scores=matrix(nrow = N,ncol=ncol(univ_efunctions[[ind]][[1]]))

    for (i in 1:N) {

      aux_scores[i,]=Variable_l2$ydiff[(ifelse(pracma::isempty(gridM[i-1]),1,sum(gridM[1:i-1])+1)):(sum(gridM[1:i]))] %*% univ_efunctions[[ind]][[i]]
    ...
    for (i in 1:N) {
      for (j in 1:npcs[[ind]]) {
        ...
        SIGMA=matrix(Variable_cov$kprod[ifelse(pracma::isempty(ndays_df$maxT[i-1]),1,sum((ndays_df$maxT[1:i-1])^2)+1):sum((ndays_df$maxT[1:i])^2)],nrow=ndays_df$maxT[i],ncol=ndays_df$maxT[i])

        Y_demeaned=as.matrix(Variable_l2$ydiff[(ifelse(pracma::isempty(ndays_df$maxT[i-1]),1,sum(ndays_df$maxT[1:i-1])+1)):(sum(ndays_df$maxT[1:i]))])

        # THESE ARE THE SCORES BY Yao et al 2005
        ee[i,j]=pc_grid$evalues[[i]][j] %*% pc_grid$efunctions[[i]][,j] %*% SIGMA %*% Y_demeaned

        ee_int[i,j]= pc_grid$efunctions[[i]][,j] %*% Y_demeaned

      }}

    EE=cbind(EE,ee)

    EE_int=cbind(EE_int,ee_int)
  ...
  Phi_m=vector(mode = "list",length = length(M_temp)) #Phi_m=array(dim = c(max(gridM), m_npcs, n_var))

  score_m=matrix(nrow = N, ncol=m_npcs)

  for (i in 1:length(gridM)) {


    Phi_m[[i]] = vector(mode = "list",length = n_var) # Phi VA A SER UNA LISTA DE LISTAS
    # LA EXTERIOR CORRE POR LOS DOMINIONS
    # LA INTERIOR CORRE POR LAS VARIABLES
    for (j in 1:n_var) {
      ...
      if(j==1){
        ...
        aux=(max(head(cumsum(npcs),j-1))+1):max(head(cumsum(npcs),j))

      }

      Phi_m[[i]][[j]] =  univ_efunctions[[j]][[i]] %*% as.matrix(c_m[[i]][aux,])

    }

    score_m[i,] = t(as.matrix(EE[i,])) %*% c_m[[i]]
    score_m_int[i,] = t(as.matrix(EE_int[i,])) %*% c_m[[i]]
